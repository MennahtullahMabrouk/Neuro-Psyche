...
class MRIClassifier:
            ...
            if file_path.suffix in [".nii", ".gz"] or ".nii.gz" in file_path.name:
                ...
                if "sub-" in file_path.name:
                    try:
                        ...
                        logging.warning(f"Could not parse subject number from {file_path.name}: {e}")
        if not file_paths:
    ...
    def build_model(self):
        model = Sequential([
            Conv3D(16, kernel_size=(3, 3, 3), activation='relu', input_shape=self.target_shape),
    ...
    def save_model(self, output_dir, model_name="mri_classifier"):
        output_dir = Path(output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)
        model_path = output_dir / f"{model_name}.h5"
        config_path = output_dir / f"{model_name}.joblib"
        self.model.save(model_path)
        joblib.dump({'target_shape': self.target_shape, 'class_names': self.class_names}, config_path)
